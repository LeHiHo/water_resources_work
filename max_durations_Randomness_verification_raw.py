...
file_path = 'C:\Program Files (x86)\환경부\RFAHD\conf\Data_Max\10071203.MAX1'
...
print("First few rows:/n", data.head())

# Extracting all hourly data into a single series for analysis, excluding the first column (Date)
...
def run_test(data):
    ...
    if std_dev == 0:
        return float('inf'), 0.0  # 표준편차가 0이면 무작위성이 아니라고 가정
